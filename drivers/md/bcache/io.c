			      ...
			      bool punt)
{
	...
	extent_for_each_ptr(e, ptr) {
		...
		if (ptr + 1 < &extent_entry_last(e)->ptr) {
			...
			n->orig			= &bio->bio.bio;
			atomic_inc(&n->orig->bi_remaining);
...
static void bch_write_endio(struct bio *bio, int error)
{
	...
	if (cache_nonfatal_io_err_on(error, wbio->bio.ca, "data write")) {
