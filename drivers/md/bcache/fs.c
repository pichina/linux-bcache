...
PAGEFLAG(Allocated, private)
TESTSCFLAG(Allocated, private)
...
{
	lockdep_assert_held(&ei->vfs_inode.i_mutex);
...
static int bch_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	lockdep_assert_held(&dir->i_mutex);
		      ...
		      struct inode *new_dir, struct dentry *new_dentry)
{
	...
	lockdep_assert_held(&new_dir->i_mutex);
	...
	if (new_inode && S_ISDIR(old_inode->i_mode)) {
		lockdep_assert_held(&new_inode->i_mutex);
...
static int bch_truncate_page(struct address_space *mapping, loff_t from)
{
	...
	page = find_lock_page(mapping, from >> PAGE_CACHE_SHIFT);
	if (!page) {
			   ...
			   struct page **pagep, void **fsdata)
{
	...
	if (pos + len >= inode->i_size) {
		 ...
		 */
		zero_user(page, offset + len, PAGE_CACHE_SIZE - offset - len);
		 ...
		 */
		if ((index << PAGE_CACHE_SHIFT) >> inode->i_size) {
			 ...
			 struct page *page, void *fsdata)
{
	...
	/*
	 * can't set a page dirty without i_mutex, to avoid racing with truncate
	 */
	lockdep_assert_held(&inode->i_mutex);
...
static void bch_direct_IO_read_endio(struct bio *bio, int error)
{
			     ...
			     loff_t offset)
{
	...
	return ((iov_iter_rw(iter) == WRITE)
		? bch_direct_IO_write
		: bch_direct_IO_read)(c, req, file, inode, iter, offset);
}
